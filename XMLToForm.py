...
def XMLToForm(s, form):
    ...
    for setting in settings:
        ...
        if value is None:
            continue
        setattr(form, setting, value.text.encode(xmlencoding)) 

    # create groups
    has_default = 0
    for group in top.first.form.first.groups.elements.group:
        # get group title and create group
        group_title = group.first.title.text.encode(xmlencoding)
        ...
        for entry in group.first.fields.elements.field:
            id = entry.first.id.text.encode(xmlencoding)
            meta_type = entry.first.type.text.encode(xmlencoding)
            try:
                form._delObject(id)
            ...
            for name in values.getElementNames():
                ...
                if value.attributes.get('type') == 'float':
                    ...
                    field.values[name] = eval(value.text.encode(xmlencoding))
                    #field.values[name] = eval(value.text)
                    #field.values[name] =  eval(unicode(value.text,'unicode-escape'))
                else:
                    field.values[name] = value.text.encode(xmlencoding)
            ...
            for name in tales.getElementNames():
                field.tales[name] = TALESMethod(
                    getattr(tales.first, name).text.encode(xmlencoding))
            ...
            if hasattr(entry.first, 'messages'):
                ...
                for entry in messages.elements.message:
                    ...
                    text = entry.text.encode(xmlencoding)
...
def oldXMLToForm(s, form):
    """Takes an xml string and changes formulator form accordingly.
    Heavily inspired by code from Nikolay Kim.
    """
    top = XMLObjects.XMLToObjectsFromString(s)
    # wipe out groups
    #encoding = top._encoding
    form.groups = {'Default':[]}
    form.group_list = ['Default']
    ...
    for setting in settings:
        ...
        if value is None:
            continue
        setattr(form, setting, value.text.encode('latin-1')) 

    # create groups
    has_default = 0
    for group in top.first.form.first.groups.elements.group:
        # get group title and create group
        group_title = group.first.title.text.encode('latin-1')
        if group_title == 'Default':
            has_default = 1
        form.add_group(group_title)
        # create fields in group
        if not hasattr(group.first.fields.elements, 'field'):
            ...
            continue
        for entry in group.first.fields.elements.field:
            id = entry.first.id.text.encode('latin-1')
            meta_type = entry.first.type.text.encode('latin-1')
            try:
                form._delObject(id)
            except (KeyError, AttributeError):
                pass
            ...
            for name in values.getElementNames():
                ...
                if value.attributes.get('type') == 'float':
                    ...
                    # XXX bare eval here (this may be a security leak ?)
                    field.values[name] = eval(value.text.encode('latin-1'))
                    #field.values[name] = eval(value.text)
                    #field.values[name] =  eval(unicode(value.text,'unicode-escape'))
                else:
                    field.values[name] = value.text.encode('latin-1')
                    #field.values[name] = value.text
            ...
            for name in tales.getElementNames():
                field.tales[name] = TALESMethod(
                    getattr(tales.first, name).text.encode('latin-1'))
            ...
            if hasattr(entry.first, 'messages'):
                ...
                for entry in messages.elements.message:
                    name = entry.attributes.get('name')
                    text = entry.text.encode('latin-1')
                    field.message_values[name] = text
